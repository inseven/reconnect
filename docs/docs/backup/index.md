...
Incremental backups can be significantly faster than full backups as they only copy modified files from your Psion. These always contain the full set of files on your Psion; unchanged files are copied from the most recent backup. They are safe (and the default behaviour) but it is important to understand the way they work if you plan to use your Psion with other backup programs like PsiWin or FastBackup.

> [!IMPORTANT]
>
> Although Reconnect implements mechanisms to detect when other programs have been used to back up a Psion (potentially invalidating the archive flag state), using incremental backups with other Psion backup programs should be considered unsafe and may result in data loss.
...
In addition to checking the [canary](#detecting-other-backup-programs), Reconnect also ensures all unmodified files exist in the most recent backup and that they are the same file size as those reported by the Psion. Only in this scenario is the last backup considered as safe source for these files, and will Reconnect perform an incremental backup.
